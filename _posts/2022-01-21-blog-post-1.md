...
Then we create our empty database in current directory called temps.db and read in the files. We clean up our data by: 

1)stacking up everything except for the indexes chosen so that the months fall in a single column. 

2)keeping only the last two characters in the column "Months" so that we can turn a word into numbers. 

3)changing the column name of the data set.
...
## 2.Then we write out the query function to present the table we desire.
...
## 3. Write a geographical scatter function for yearly temperature
...
The following plot displays "Monthly Changes in Teuntry, start_year,end_year))
...
mperature in different Years and stations" in a specific country we choose. 
...
def LinePicture(country, start_year,end_year):
    ...
    order by T.year
    """
    df = pd.read_sql_query(cmd, conn, params = (co
